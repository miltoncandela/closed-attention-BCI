...
# changes.
def timer(second, count, timestamps, tiempo_prueba):
...
# # CODE FOR EEG # #
def EEG(second, folder, tiempo_prueba):
    # PSD 128 rows
    ...
    try:
        while (True):
            ...
            for eeg_channel in eeg_channels:
                ...
                # Afterward, a for loop goes over all the spectral signals and saves the unique
                # values into a list, possible because only 1 value is generated per second.
...
def MachineLearning(second, folder, queue, tiempo_prueba):
        ...
        with second.get_lock():
            ...
            if (second.value > 91) and (second.value % 10 == 0) and (scaler is not None) and (current_second != second.value):
                ...
                if df_current.shape[0] > 0:
                    df_current['Pred'] = (df_current.Beta / (df_current.Theta + df_current.Alpha))*100
...
async def bluetooth_worker(second, queue, device_address, tiempo_prueba):
...
def start_bluetooth_thread(second, queue, device_address, tiempo_prueba):
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    loop.run_until_complete(bluetooth_worker(second, queue, device_address, tiempo_prueba))
    loop.close()
...
if __name__ == '__main__':
    ...
    tiempo_prueba += offset[int(repetition_num)-1]
    print('Tiempo total:', tiempo_prueba, 'segundos')
    os.mkdir(folder)
    ...
    # # Start processes # #
    process2 = Process(target=timer, args=[seconds, counts, timestamps, tiempo_prueba])
    blue = threading.Thread(target=start_bluetooth_thread, args=(seconds, queue, device_address, tiempo_prueba))
    # blue = Process(target=start_bluetooth_process, args=(queue,))
    q = Process(target=EEG, args=[seconds, folder, tiempo_prueba])
    m = Process(target=MachineLearning, args=[seconds, folder, queue, tiempo_prueba])
    process2.start()
    q.start()
    if repetition_num != '1':
        blue.start()
